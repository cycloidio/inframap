...
# inframap archlinux package to the AUR
# usage:
# ```
...
tag_name=$(curl --silent "https://api.github.com/repos/cycloidio/inframap/releases/latest" |
	jq -r .tag_name
)

source_i386="https://github.com/cycloidio/inframap/releases/download/$tag_name/inframap-linux-386.tar.gz"
source_amd64="https://github.com/cycloidio/inframap/releases/download/$tag_name/inframap-linux-amd64.tar.gz"
...
# compute sha256 sums
sha256sum_i386=$(sha256sum inframap-linux-386.tar.gz | awk '{ print $1 }')
sha256sum_amd64=$(sha256sum inframap-linux-amd64.tar.gz | awk '{ print $1 }')
...
GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no -i ./id_rsa.aur" git clone ssh://aur@aur.archlinux.org/inframap.git && cd inframap
